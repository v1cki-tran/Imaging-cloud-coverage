...
def csvs(directory):
	...
	return csv_writer_sensor, csv_writer_image_cloud
	...
	for cols in headings:	
		if plot_dict[cols]:
			if len(csvfile[headings[n]])<2:
				...
				csvfile.plot(0,cols)
				plt.ylabel(cols) # make the plot, this needs the most work as the x axis always looks fucked up from the timestamps
...
def cloud_plots(csv, directory):
	...
	plot_bools = [0,0]
	plot_bools.extend([cloudcover.get(), avgmag.get(), direction.get()]) # no idea how we should plot direction over time
	tuple(plot_bools)
...
def Run():
	...
	csv_writer_sensor, csv_writer_image_cloud = csvs(cwd)
	now = datetime.now()
	...
	__,file_name = capture_image(camera, masked, mask, cwd)
...
cloudcover_check = ttk.Checkbutton(master=plot_frame, text = 'Cloud cover(%)', style = 'Roundtoggle.Toolbutton', variable=cloudcover)
avgmag_check = ttk.Checkbutton(master=plot_frame, text = 'Average magnitude', style = 'Roundtoggle.Toolbutton', variable=avgmag)
direction_check=ttk.Checkbutton(master=plot_frame, text = 'Direction', style = 'Roundtoggle.Toolbutton', variable=direction)
...
cloudcover_check.pack()
avgmag_check.pack()
direction_check.pack()
