...
def cloud_detection(file_name, masked, mask_path):
    ...
    if (np.sum(sun_mask) / 255) > 100:
        ...
        black_mask = cv2.bitwise_and(black_mask, black_mask, mask=(255-sun_detection))
    ...
    #geom_weight_map = 1/geom_weight_map
    
    
    weighted_image = combined_agree_mask * geom_weight_map
    weighted_image_math = np.sum(weighted_image)
    useful_math = np.sum(black_mask*geom_weight_map)
    percent_cover_weighted = round((np.sum(weighted_image_math) / useful_math) * 100, 2) if useful_pixels > 0 else 0
    #print("Weighted percent: ", percent_cover_weighted)
    
    return y_channel, percent_cover_weighted, file_name, motion_mask
    ##################################################################################
    ##################################################################################
"""
    plt.figure(dpi=150)
