...
try:
	camera = Picamera2()
	config = camera.create_preview_configuration(main={"size": (700,525)})
	camera.configure(config)
	camera.start()
except IndexError:
	print('Camera is disconnected! Plug it in and reboot the pi!')
	tk.messagebox.showerror("Error!", 'The camera cable is loose or disconnected! reconnect it and restart the system!')
...
def stopcam():
    ...
    camera_button.config(text="""
    Open Camera
    """, command=cam_function)
...
def main():
    global img_copy, points, masked, mask, mask_path
...
def csvs(directory):
	...
	csvfile = open((datetime.now().strftime('%Y-%m-%d') + '_data.csv'), 'a', newline='')
	csv_writer = csv.writer(csvfile)

	# Add headers to the CSV files if empty
	if csvfile.tell() == 0:
		csv_writer.writerow(['Timestamp', 'Temp(C)', 'Humidity(%)', 'Pressure(hPa)', 'Pressure(PSI)', 'Dew Point', 'Dew Point Depression', 'Cloud Ceiling(m)', 'Image Filename', 'Cloud Coverage (%)', 'Avg Magnitude', 'Direction'])

	return csv_writer, csvfile

def plots(csv, directory):
	print("Started plotting weather")
	global temp, humidity, pressure1, pressure2, dewpoint, dewpointdep, cloudc, cloudcover, avgmag, direction # Call variables from checkboxes
	...
	pressure2.get(), dewpoint.get(), dewpointdep.get(), cloudc.get(), 0,cloudcover.get(), avgmag.get(), 0])
	...
	for cols in headings:	
		if plot_dict[cols]:
			if len(csvfile[headings[n]])<2:
				...
				date, time = csvfile[headings[0]][1].split(" ")
				plt.title(f"{cols} {date}")
...
def plot_direction(csv, directory):
	...
	for direc in csvfile[headings[11]]:
		if direc.lower() == 'north':
			...
			u.append(-1)
			v.append(0)
		elif direc.lower() == 'north-west':
			u.append(-1)
			v.append(1)
		elif direc.lower() == 'south-west':
			u.append(-1)
			v.append(-1)
		elif direc.lower() == 'north-east':
			u.append(1)
			v.append(1)
		elif direc.lower() == 'south-east':
			u.append(1)
			v.append(-1)
	...
	plt.locator_params(axis='x', nbins=8)
	date, time = csvfile[headings[0]][1].split(" ")
	plt.title(f"Direction and magnitude {date}")
...
def button_serial():
	serial_label.configure(text=f"""Timestamp, Temp(C), Humidity(%), Pressure(hPa), Pressure(PSI), Dew Point, Dew Point Depression, Cloud Ceiling(m)
	{read_serial_data(ser, pin, [])}""")
...
def ready_configuration():
	more_duration.configure(command=morehours)
	less_duration.configure(command=lesshours)
	
	
	camera_button.configure(command=cam_function)
	crop_button.configure(command=main)
	
	more_interval.configure(command=moresecs)
	less_interval.configure(command=lesssecs)
	
	file_button.configure(command=change_directory)
	
	more_fps.configure(command=morefps)
	less_fps.configure(command=lessfps)
...
def Run():
	...
	input_image1, _, _, ___ = cloud_detection(file_name, masked, mask_path)
	...
	while now <= stop_time:
		...
		if now >= interval:
			...
			input_image2, mean_cover, image_filename, motion_mask = cloud_detection(file_name, masked, mask_path)
			csv_array = cloud_motion(input_image1, input_image2, mean_cover, image_filename, now, csv_array, motion_mask)
			
			print(f"Total line: {csv_array}")
	...
	plots(os.path.join(cwd, datetime.now().strftime('%Y-%m-%d') + '_data.csv'), cwd)
	print("Done plotting!")
...
#notebook.configure('Tbutton', font=('Helvetica',12))
##########################
...
#run_frame=ttk.Frame(master=tab1, height=100, width=100)
#run_frame.pack_propagate(0)
run_button=ttk.Button(master=run_frame, text="""
     Run     
""", command = threading)
...
camera_button = ttk.Button(master=camera_frame, text=
"""
Open Camera
""", command = cam_function)
crop_button = ttk.Button(master=camera_frame, text=
"""
Crop Image
""", command = main)
...
file_button = ttk.Button(master = file_frame, text = """
Change...
""", command = change_directory)

file_label.pack()
file_button.pack(side='right', pady=10)

file_frame.pack()
#################################
# Time lapse frame
timelapse_frame = ttk.Frame(master=tab2)
timelapse_label = ttk.Label(master = timelapse_frame, text= f"Time lapse fps:{fps} ({((hour*3600)/intervalInSeconds)/fps}s)", font='Calibri 24')
...
timelapse_label.pack(side='left', padx=10, pady=10)
...
serial_button =ttk.Button(master=serial_frame, text="""
Read weather data
""",
                          command = button_serial)
...
##################################
# Plot Options menu!
plot_menubutton=ttk.Menubutton(master=tab2, text="""
graphs to plot
""")
...
pressure2 = tk.IntVar(master=window, value = 0)
dewpoint = tk.IntVar(master=window, value = 0)
...
direction = tk.IntVar(master=window, value = 1)
